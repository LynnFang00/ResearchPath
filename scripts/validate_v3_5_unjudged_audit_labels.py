...
DEFAULT_LABELS = REPO_ROOT / "data" / "eval" / "manual_labels_v3_5_unjudged_audit.jsonl"
DEFAULT_CANDIDATES = REPO_ROOT / "data" / "eval" / "labeling" / "v3_5_unjudged_top10_audit_candidates.jsonl"
...
DEFAULT_JSON_OUT = REPO_ROOT / "data" / "eval" / "results" / "v3_5_unjudged_audit_label_progress.json"
DEFAULT_MD_OUT = REPO_ROOT / "data" / "eval" / "results" / "v3_5_unjudged_audit_label_progress.md"
...
SCHEMA_VERSION_VALUES = {"v3.5_unjudged_audit_manual_label", "v3.2_800_manual_label"}
...
def validate_enums(label: dict[str, Any], line_number: int, invalid_values: list[dict[str, Any]]) -> None:
    ...
    if "primary_role" in label and primary_role not in ROLE_VALUES:
        invalid_values.append({"line": line_number, "field": "primary_role", "value": primary_role, "expected": sorted(ROLE_VALUES)})
...
def validate_labels(
    *,
    ...
    existing_keys = load_keys(v21_labels) | load_keys(v25_labels) | load_keys(v32_labels)
    ...
    hash_checks = {
        ...
        "manual_labels_v3_2_sha256": v32_hash,
        "manual_labels_v3_2_unchanged": v32_hash == EXPECTED_V32_LABELS_SHA256,
        "selected_240_sha256": selected_hash,
        "selected_240_unchanged": selected_hash == EXPECTED_SELECTED_240_SHA256,
    }
    return {
        "schema_version": "v3.5_unjudged_audit_label_progress",
        ...
        "unlabeled_candidate_count": len(candidate_keys - labeled_keys),
        ...
        "complete": len(labeled_keys & candidate_keys) == len(candidate_keys),
        "is_valid": schema_valid and all(hash_checks.values()),
    }


def write_json_file(path: Path, report: dict[str, Any]) -> None:
...
def build_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# V3.5 Unjudged Audit Label Progress",
        "",
        f"Created: `{report['created_at']}`",
        "",
        f"- Labels: `{report['labeled_candidate_count']}` / `{report['candidate_count']}`",
        f"- Unlabeled: `{report['unlabeled_candidate_count']}`",
        ...
        f"- Labels not in V3.5 candidates: `{len(report['labels_not_in_candidates'])}`",
        f"- Overlap with V2.1/V2.5/V3.2 labels: `{len(report['overlap_existing_labels'])}`",
        "",
        "## Per-Topic Progress",
        "",
    ...
    lines.extend(
        [
            ...
            f"- V3.2 labels unchanged: `{report['hash_checks']['manual_labels_v3_2_unchanged']}`",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate additive V3.5 unjudged-audit manual labels.")
...
def main() -> None:
    ...
    report = validate_labels(
        labels=load_jsonl(resolve_repo_path(args.labels), missing_ok=True),
        candidates=load_jsonl(resolve_repo_path(args.candidates)),
        v21_labels=load_jsonl(resolve_repo_path(args.v21_labels)),
        v25_labels=load_jsonl(resolve_repo_path(args.v25_labels)),
        v32_labels=load_jsonl(resolve_repo_path(args.v32_labels)),
    ...
    write_json_file(resolve_repo_path(args.json_out), report)
    ...
    print(f"V3.5 labels: {report['labeled_candidate_count']} / {report['candidate_count']}")
